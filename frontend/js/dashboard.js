/**
 * Dashboard functionality - complete dashboard application logic
 * Main application logic for the topics and events dashboard
 */

import { apiBase, getToken, authHeaders, clearToken } from './config.js';

// Global state and initialization
const topicState = {};
const sortState = { field: 'date', direction: 'asc' };
...
function renderTopics(topics, sourcesByTopic, eventsByTopic, countsByTopic) {
      ...
      <div class="card-body">
        <div class="topic-header d-flex justify-content-between align-items-center" id="topic-header-${t.id}" role="button" tabindex="0" title="Click to expand or collapse this topic">
          <div class="d-flex align-items-center flex-grow-1">
            <div class="icon-placeholder icon-topic me-3">📑</div>
        ...
        <div class="collapse show" id="topic-content-${t.id}">
          <div class="d-flex justify-content-between align-items-center mt-4 mb-3">
            <h3 class="h6 mb-0 d-flex align-items-center">
              <div class="icon-placeholder icon-topic me-2">📡</div>
          ...
          <div class="mb-3">
            <div class="events-header d-flex justify-content-between align-items-center mb-3">
              <h3 class="h6 mb-0 d-flex align-items-center">
                <div class="icon-placeholder icon-event me-2">📅</div>
    ...
    `;
        ...
        sources.forEach((s, index) => {
            ...
            if (lastScrapedDate) {
                const minutesAgo = Math.floor((now - lastScrapedDate) / (1000 * 60));
                if (minutesAgo < 60) {
                    statusIndicator = `🟢 Active • Last: ${minutesAgo}m ago`;
                    statusClass = 'status-active';
                } else if (minutesAgo < 1440) { // 24 hours
                    const hoursAgo = Math.floor(minutesAgo / 60);
                    statusIndicator = `🟡 Recent • Last: ${hoursAgo}h ago`;
                    statusClass = 'status-recent';
                } else {
                    const daysAgo = Math.floor(minutesAgo / 1440);
                    statusIndicator = `🔴 Stale • Last: ${daysAgo}d ago`;
            ...
            row.innerHTML = `
                  <div class="feed-content">
                      <div class="d-flex align-items-center">
                          <div class="icon-placeholder ${iconClass} me-3">${iconMap[s.source_type] || '🔗'}</div>
                          <div class="flex-grow-1">
                              <div class="d-flex align-items-center gap-2 mb-1">
                                  <span class="fw-semibold">${s.name}</span>
                                  <span class="badge text-bg-light">${s.source_type}</span>
                                  <span class="feed-status ${statusClass}">${statusIndicator}</span>
                              </div>
        ...
        const statsText = [];
        if (sourcesCount > 0) statsText.push(`📡 ${sourcesCount} feed${sourcesCount > 1 ? 's' : ''}`);
        if (totalCount > 0) statsText.push(`📅 ${totalCount} event${totalCount > 1 ? 's' : ''}`);
        topicStatsEl.textContent = statsText.length > 0 ? `• ${statsText.join(' • ')}` : '';
        ...
        collapseIndicator.textContent = topicState[t.id].isExpanded ? '▼' : '▶';
        ...
        controlsBtn.onclick = () => {
            ...
            if (isOpen) {
                ...
                controlsBtn.textContent = '⚙️ Options';
            } else {
                controlsPanel.classList.add('show');
                controlsBtn.textContent = '⚙️ Hide Options';
            }
        };
        ...
        if (isMinimalView && !expandedInMinimal) {
            ...
            eventDiv.innerHTML = `
                  <div class="d-flex align-items-center gap-3">
                    <div class="icon-placeholder icon-event flex-shrink-0">📅</div>
                    <div class="flex-grow-1 min-w-0">
                      <h4 class="h6 mb-1 fw-semibold">${ev.title}</h4>
                      <div class="small text-secondary">
                        <span class="fw-medium">📅 ${dateStr}</span>
                        ${ev.location ? ` • <span class="fw-medium">📍 ${ev.location}</span>` : ''}
                      </div>
                    </div>
                    ...
                    <div class="flex-grow-1 min-w-0">
                      ...
                      ${(!isMinimalView || (isMinimalView && expandedInMinimal)) ? `<div class="event-expand-hint text-end">
                        <small class="text-muted cursor-pointer event-sources-toggle" data-event-id="${evId}">
                          ${expanded ? '👁️ Click to hide sources' : '👁️ Click to show sources'}
                        </small>
                      </div>` : ''}
                    </div>
                  </div>
    ...
    extracted.forEach(x => {
        ...
        sourceItem.innerHTML = `
              <div class="d-flex align-items-start">
                <div class="icon-placeholder icon-web me-2">🔗</div>
                <div class="flex-grow-1">
                  ...
                  <div class="small text-secondary">
                    📅 Published ${dateStr} • 🔗 hops: ${x.degrees_of_separation}
                  </div>
...
function setupCollapseAllSwitch() {
    const collapseAllSwitch = document.getElementById('collapse-all-switch');
    const collapseAllLabel = document.querySelector('label[for="collapse-all-switch"]');
    ...
    collapseAllSwitch.onchange = () => {
        ...
        allTopicIds.forEach(topicId => {
            ...
            if (collapseIndicator) {
                collapseIndicator.textContent = shouldExpand ? '▼' : '▶';
            }
            ...
            if (eventsSwitch) {
                ...
                if (eventsLabel) eventsLabel.textContent = shouldExpand ? 'Expand' : 'Collapse';
            }
        });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Setup global collapse/expand switch
    setupCollapseAllSwitch();
